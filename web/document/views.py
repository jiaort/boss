...
from web.document.forms import JobForm, JobListForm
...
def incoming_list_view(request):
    ...
    data = {
        'active_classes': ['.incoming-menu'],
        'open_classes': ['.document-menu'],
    ...
    try:
        ...
        return render_admin(request, 'document/incoming_list.html', data)
...
def outgoing_list_view(request):
    ...
    try:
        ...
        SysLogger.exception(ex, request)
        messages.error(request, '系统异常，请重试', 'danger')
        return render_admin(request, 'document/outgoing_list.html', data)
...
def job_view(request, job_id=None):
    ...
    data = {
        ...
        'job_status_option': JobStatus.CHOICES,
        'publish_status_option': PublishStatus.CHOICES,
        'channel_option': PublishChannel.CHOICES,
        'function_type_option': FunctionType.CHOICES,
        'job_type_option': JobType.CHOICES,
        'sub_job_type_option': SubJobType.CHOICES,
        'work_place_option': WorkPlace.CHOICES,
        'sub_work_place_option': SubWorkPlace.CHOICES,
        'salary_option': Salary.CHOICES,
        'education_option': Education.CHOICES,
        'experience_option': Experience.CHOICES,
        'email_option': EMAIL.CHOICES,
        'job_info': job_info,
    }

    return render_admin(request, 'job/new.html', data)


@login_check_ajax()
def edit_view(request, job_id=None):
    ...
    try:
        ...
        if job_id:
            Job.objects.filter(id=job_id).update(**data)
        else:
